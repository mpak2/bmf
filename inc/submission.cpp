...
int main(int argc, char **argv){
	...
	//}else if(mpre("Проверка "+ file_name, __LINE__); false){ pre("Уведомление");
			...
			//}else if(CSV.insert(make_pair(CSV.size(), csv)); CSV.empty()){ mpre("ОШИБКА добавления новой строки", __LINE__);
				...
				}else if(2 >= argc){ //mpre("Остаток не указан ", __LINE__); // assert(!("Длина не может быть отрицательна", 1011)); //assert(!"Проверяем на ноль"); //assert(!"ОШИБКА какая то"); //throw __LINE__;// ohrow "Ошибка - на 0 делить нельзя!!!!"; //std::perror("ОШИБКА"); //mpre("Делитель не задан", __LINE__);
				}else if(split = atoi(argv[2]); (0 > split)){ mpre("ОШИБКА выборки делителя", __LINE__);
				}else if(3 >= argc){ //mpre("Делитель не указан ", __LINE__); // assert(!("Длина не может быть отрицательна", 1011)); //assert(!"Проверяем на ноль"); //assert(!"ОШИБКА какая то"); //throw __LINE__;// ohrow "Ошибка - на 0 делить нельзя!!!!"; //std::perror("ОШИБКА"); //mpre("Делитель не задан", __LINE__);
				}else if(inc = atoi(argv[3]); false){ mpre("ОШИБКА расчета коррекции", __LINE__);
			...
			}else if(std::string cmd = "./bimorph "+ dbname+ " -j '" +data +"' 2>/dev/null"; cmd.empty()){ mpre("ОШИБКА формирования строки комманды расчета", __LINE__);
			//}else if("1612" != id){ mpre("Пропускаем 1612", __LINE__);
				...
				} return result; }(); result.empty()){ mpre("ОШИБКА данные расчета пусты", __LINE__);
				...
				}else if(price = json[id]["SalePrice(Отпускная цена дома)"]; price.empty()){ mpre("ОШИБКА выборки расчетной цены", __LINE__);
				...
				}else if(sql = "CREATE TABLE IF NOT EXISTS `mp_bmf_submission` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE, `time` INTEGER, `_id` INTEGER, `price` INTEGER)"; sql.empty()){ mpre("ОШИБКА установки строки запроса", __LINE__);
				}else if(SQLITE_OK != sqlite3_exec(db, sql.c_str(), 0, &stmt, 0)){ mpre("ОШИБКА выполнения запроса " +sql, __LINE__);
				}else if(sql = "INSERT OR REPLACE INTO `mp_bmf_submission` (id, _id, time, price) VALUES ((SELECT `id` FROM `mp_bmf_submission` WHERE `_id`="+ id+ "), '"+ id+ "','"+ std::to_string(std::time(0))+ "','"+ price+ "');"; sql.empty()){ mpre("ОШИБКА установки строки запроса", __LINE__);
				}else if(SQLITE_OK != sqlite3_exec(db, sql.c_str(), 0, &stmt, 0)){ mpre("ОШИБКА выполнения запроса " +sql, __LINE__);
