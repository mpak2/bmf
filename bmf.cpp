...
int main(int argc, char **argv){
	...
	std::function<int(json,int,int,string,TM3i&,TM3i&)> LearningAll; // Обучение
	std::function<int(TMs,TM3i&,TM3i&)> Learning; // Обучение
	std::function<string(double)> Dec2bin; // Перерасчет размерности размерности из десятеричной в двоичную
	std::function<string(TMs,string,TM3i&,TM3i&)> Learn; // Рерасчет морфа
	...
	if([&](){ // Список значений из консоли
			if([&](){ for(int i = 0; i < argc; i++){ // Параметры значений консоли
					...
					if(0 >= i){ //mpre("Имя файла", __LINE__);
					}else if([&](){ arg = argv[i]; return (0 >= arg.length()); }()){ mpre("ОШИБКА получения аргумента консоли", __LINE__);
						...
						}()){ mpre("ОШИБКА установки БД", __LINE__);
					...
					}else if([&](){ val = argv[i+1]; return (0 >= val.length()); }()){ mpre("ОШИБКА получения значения параметра "+ arg + " "+ to_string(i+1), __LINE__);
		...
		if(ARGV.end() == ARGV.find("-")){ //mpre("Стандартный вывод не задан", __LINE__);
			...
			if([&](){ // Отображение атрибутов командной строки
				...
				}()){ mpre("Атрибуты параметров командной строки", __LINE__);
			}else if([&](){ // Раздел помощи
					string file;
					if([&](){ file = (1 <= argc ? argv[0] : "./bimorph"); return (0 >= file.length()); }()){ mpre("ОШИБКА получения имени файла", __LINE__);
				...
				return false; }()){ mpre("Раздел помощи", __LINE__);
					...
					if(ARGV.end() == ARGV.find("-v")){ return false; //mpre("Пропускаем отображение версии", __LINE__);
						...
						std::cout << "Авторы программы -- Кривошлыков Евгений Павлович +7 911 9842884" << endl << endl;

					} return true;
				}()){ mpre("Информация о версии", __LINE__);
			}else{
				return false;
			} return true;
		}()){ exit(mpre("Остановка выполнения", __LINE__));
			...
			if(ARGV.end() == ARGV.find("-d")){ mpre("ОШИБКА БД для сохранения не задана -d", __LINE__);
			...
			}else if([&](){ dbname = ARGV.at("-d"); return (0 >= dbname.length()); }()){ mpre("ОШИБКА имя файла для БД не задано", __LINE__);
			}else if([&](){ // Запрос к БД
				...
				return false; }()){ mpre("ОШИБКА установки функции выборки списка таблциы", __LINE__);
					...
					Dataset = ([&](TMMi& TAB, string name, string table, string comment){ // Функция восстановления из БД с проверкой
						...
						//}else if(true){ mpre(argv[0], __LINE__);
						//}else if(argv[0] != "./bmf"){ mpre("Не проверяем данные таблиц", __LINE__);
			...
			if([&TIMER, &Timer](){ Timer = ([&](string name, string title, int line){ // Получаем число возвращаем двойное числоa
					...
					Values = ([&](TMs value, string alias, TM3i &BMF_VALUES, string clump_id){ // Проверка наличия значений в БД и установка новых
						...
						for(auto &value_itr:value){
							if([&](){ _values = value_itr.first; return (0 >= _values.length()); }()){ mpre("ОШИБКА получения имени значения", __LINE__);
									...
									std::lock_guard<std::recursive_mutex> lock(mu);
									if([&](){ values = erb(BMF_VALUES, {{"name", _values}}); return !values.empty(); }()){ //mpre("Значение уже создано `"+ _values+ "`", __LINE__);
									//}else if(ARGV.end() != ARGV.find("-t")){ mpre("ОШИБКА в многопоточном режиме запрещено добавление новых значений", __LINE__);
									}else if([&](){ values = {{"id", Id(BMF_VALUES.at(""))}, {"name", _values}, {"clump_id", clump_id}, {"value", _val}}; return values.empty(); }()){ mpre("ОШИБКА фонмирования нового значения", __LINE__);
									}else if([&](){ erb_insert(BMF_VALUES, values["id"], values); return BMF_VALUES.empty(); }()){ mpre("ОШИБКА добавления нового значения в справочник", __LINE__);
				...
				return false; }()){ mpre("ОШИБКА установки функции установки значений", __LINE__);
					...
					Vals = ([&](TMs VALUE, string alias, int key, TM3i& BMF_VALS, TM3i& BMF_VALUES, TM3i& BMF_TITLES, string clump_id){ // Установка входных значений
						static TM3i DANO; bool cache = true; clock_t t;
						if(false){ mpre("ОШИБКА уведомления", __LINE__);
							...
							}()){ //mpre("Кеширование "+ to_string(key), __LINE__);
						}else if([&](){ // Расчет знаков
							int bmf_vals_size = BMF_VALS.at("").size();
							for(auto &val_itr:VALUE){ //mpre("VALUE ", __LINE__);
								TMMi VALS; TMs values, bmf_titles; string val, _value, _value_, value; long long bin; int pos;
								if([&](){ value = val_itr.second; return false; }()){ mpre("ОШИБКА получения значения итератора", __LINE__);
								//}else if([&](){ mpre("VALUE ("+ value+ ") "+ alias, __LINE__); return false; }()){ mpre("ОШИБКА уведомления", __LINE__);
									...
									}else if(TITLES = rb(BMF_TITLES, {{alias+ "_values_id", values["id"]}}); false){ mpre("ОШИБКА получения полного списка", __LINE__);
									...
									} return false; }()){ mpre("ОШИБКА получения заголовка", __LINE__);
									...
									if(value = (bmf_titles.empty() ? value : bmf_titles.at("value")); (0 >= value.length())){ mpre("ОШИБКА Значение не определено", __LINE__);
										...
										for(int i = 0; i < _value_.length(); i++){ // Добавление знаков значению
											...
											if(val = _value_.substr(i, 1); (1 != val.length())){ mpre("ОШИБКА получения символа "+ to_string(i), __LINE__);
											...
											}else if([&](){ // Проверка и выборка глобального знака
												std::lock_guard<std::recursive_mutex> lock(mu);
											...
											}else if([&](){ // Синхронизация глобального значения с локальным
												...
												} return false; }()){ mpre("ОШИБКА Добавления локального знака", __LINE__);
		...
		return true; }()){ mpre("ОШИБКА установки списка функций", __LINE__);
			...
			if(1 > argc){ mpre("Параметров для теста не достатоно", __LINE__);
					...
					TMs index;
					if([&](){ index = {{"id", Id(BMF_INDEX_EX.at(""))}, {"index_id", "1"}, {"calc_pos_id", "2"}}; return index.empty(); }()){ mpre("ОШИБКА создания нового элемента", __LINE__);
					}else if([&](){ erb_insert(BMF_INDEX_EX, index.at("id"), index); return index.empty(); }()){ mpre("Добавление элемента к списку", __LINE__);
					}else if([&](){ index = {{"id", Id(BMF_INDEX_EX.at(""))}, {"index_id", "3"}, {"calc_pos_id", "2"}}; return index.empty(); }()){ mpre("ОШИБКА создания нового элемента", __LINE__);
					}else if([&](){ erb_insert(BMF_INDEX_EX, index.at("id"), index); return index.empty(); }()){ mpre("Добавление элемента к списку", __LINE__);
					}else if([&](){
							TMs values, _index;
							if([&](){ values = {{"index_id", "1"}}; return values.empty(); }()){ mpre("ОШИБКА установки значений выборки", __LINE__);
						...
						}()){ mpre("ОШИБКА тест не пройден", __LINE__);
					}else if([&](){
							TMs values; TMMi INDEX;
							if([&](){ values = {{"calc_pos_id", "2"}}; return values.empty(); }()){ mpre("ОШИБКА установки значений выборки", __LINE__);
							}else if([&](){ INDEX = rb(BMF_INDEX_EX, values); return INDEX.empty(); }()){ mpre("ОШИБКА выборки", __LINE__);
							}else if(2 != INDEX.size()){ mpre("ОШИБКА количество значений не верное", __LINE__);
							}else{ mpre(values, __LINE__, "Условия"); mpre(INDEX, __LINE__, "Результат"); mpre("Тест пройден корректно", __LINE__);
								return false;
							} return true;
						}()){ mpre("ОШИБКА тест не пройден", __LINE__);
					//}else if([&](){ mpre(BMF_INDEX_EX, __LINE__, "Справочник"); return false; }()){ mpre("ОШИБКА уведомления", __LINE__);
					...
					//}else if([&](){ mpre(index, __LINE__, "Элемент"); mpre(BMF_INDEX_EX, __LINE__, "Переиндексирование"); return false; }()){ mpre("ОШИБКА уведомления", __LINE__);
					}else{ //mpre(index, __LINE__, "Морф"); ;
						return false;
					} return true;
				}()){ mpre("ОШИБКА Проверка тестов работы ключей", __LINE__);
		...
		}()){ mpre("ОШИБКА не все тесты пройдены", __LINE__);
			...
			if(false){ mpre("ОШИБКА уведомления", __LINE__);
				...
				Learn = ([&](TMs bmf_index, string val = "", TM3i& _BMF_DANO_EX, TM3i& _BMF_ITOG_EX){ //mpre("Обучение "+ index.at("id"), __LINE__);  //system("sleep 1"); // mpre(BMF_INDEX, __LINE__, "Список"); // if(remove){ mpre(row, __LINE__, "Ученик"); }
					TMMi STAIRS; TMs stair, stairs; int rep = 0, size;
					if([&](){ // Требование к морфу
						if(stairs["index_id"] = bmf_index.at("id"); stairs.empty()){ mpre("ОШИБКА добавления номера идентификатора", __LINE__);
					...
					}else if([&](){ STAIRS.insert(make_pair(STAIRS.size(), stairs)); return STAIRS.empty(); }()){ mpre("ОШИБКА устанвоки первоначального значения лестнице", __LINE__);
					//}else if([&](){ mpre("До изменений promise="+ val, __LINE__); Tree(bmf_index, _BMF_DANO_EX); return false; }()){ mpre("ОШИБКА уведомления", __LINE__);
					}else if([&](){ do{ //mpre("Повтор "+ to_string(rep), __LINE__); // Ступени расчета
							TMs stairs_next, _index, index_next, index_other, index_1, index_0, calc_pos, dano; auto stairs_itr = STAIRS.rbegin(); string index_field;
							if(STAIRS.empty()){ mpre("ОШИБКА лестница пуста", __LINE__);
							}else if(!(size = STAIRS.size())){ mpre("ОШИБКА расчета размера сети", __LINE__);
								...
								} return calc_pos.empty(); }()){ mpre("ОШИБКА выборки позиции", __LINE__);
							...
							}else if([&](){ // Получение младшего потомка
								//std::lock_guard<std::recursive_mutex> lock(mu);
								...
								//}else if([&](){ mpre(stairs, __LINE__, "Ступень"); return false; }()){ mpre("ОШИБКА уведомления", __LINE__);
								...
								}else if(string::npos == stairs.at("status").find_last_of("234")){ //mpre(stairs["index_id"]+ " Не расчитываем результат для статуса "+ stairs["status"], __LINE__);
							...
							/*}else if([&](){ // Балансировка дерева
								...
								} return false; }()){ mpre("ОШИБКА формирования следующей ступени", __LINE__);
								...
								if(stairs.end() == stairs.find("promise")){ //mpre(stairs["index_id"]+ " Обещание не установлено", __LINE__);
									...
									}else if(dano = erb(_BMF_DANO_EX, {{"id", index.at("dano_id")}}); dano.empty()){ mpre("ОШИБКА получения исходника морфа", __LINE__);
						...
						}while((1e9 > ++rep) && (0 < STAIRS.size())); return false; }()){ mpre("ОШИБКА получения результатов расчета", __LINE__);
				...
				Learning = ([&](TMs itog, TM3i& _BMF_DANO_EX, TM3i& _BMF_ITOG_EX){ //mpre("Обновление", __LINE__); // Обучение
					TMs index, _index, values, bmf_itog; int change = 0;
					if(itog.empty()){ mpre("ОШИБКА итог не задан", __LINE__);
					...
					}else if([&](){ // Проверка на разные ссылки сетей у итогов
						std::lock_guard<std::recursive_mutex> lock(mu);
						...
						}else if(bmf_itog = BMF_ITOG_EX.at("").at(stoi(itog.at("id"))); bmf_itog.empty()){ mpre("ОШИБКА выборки глобального морфа", __LINE__);
						}else if(index = erb(BMF_INDEX_EX ,{{"id", bmf_itog["index_id"]}}); !index.empty()){ //mpre("Связь с морфом", __LINE__);
						//}else if([&](){ return (0 >= _BMF_DANO_EX.at("").size()); }()){ mpre("ОШИБКА список исходных данных пуст", __LINE__);
						...
						} return index.empty(); }()){ mpre("ОШИБКА морф связи не найден", __LINE__);
					}else if([&](){ // Значение
						std::lock_guard<std::recursive_mutex> lock(mu);
						...
						} return values.empty(); }()){ mpre("ОШИБКА получения значения", __LINE__);
					}else if([&](){ // Обучение если не режим расчета
						if(string _val = Learn(index, itog.at("val"), _BMF_DANO_EX, _BMF_ITOG_EX); (1 <= _val.length() >= 2)){ mpre("ОШИБКА обучения морфа значение не верная длинна результата", __LINE__);
					...
					//}else if([&](){ mpre("Окончание Learning "+ itog.at("id"), __LINE__); return false; }()){ mpre("ОШИБКА", __LINE__);
				...
				LearningAll = ([&](json js, int thread, int _thread, string clump_id, TM3i& _BMF_DANO_EX, TM3i& _BMF_ITOG_EX){ //mpre("ОбновлениеОбщее", __LINE__); // Обучение
					int change = 0;
					if(false){ mpre("Остановка выполнения LearningAll", __LINE__);
						...
						for(auto& itog_itr:_BMF_ITOG_EX.at("")){
							...
							//}else if(true){ mpre("Запуск потока " +to_string(_thread), __LINE__);
							...
							}else if(change += Learning(itog, _BMF_DANO_EX, _BMF_ITOG_EX); false){ mpre("ОШИБКА запуска расчета", __LINE__);
					...
					Do = ([&](int thread, int _thread){ // Цикл повторений расчета
						...
						if([&](){ // Проверка количества эпох из командной строки
								...
								}else if([&](){ _loop_max = (ARGV.end() == ARGV.find("-e") ? "1" : ARGV.at("-e")); return (0 >= _loop_max.length()); }()){ mpre("ОШИБКА количество эпох не задано", __LINE__);
								...
								}else if([&](){ loop_max = stoi(_loop_max); return (0 >= loop_max); }()){ mpre("ОШИБКА количество эпох не достаточно для продолжения", __LINE__);
							...
							return (0 >= loop_max); }()){ mpre("ОШИБКА количество эпох задано не верно -e", __LINE__);
								...
								do{ // Расчет и обучение входящих параметров
									if([&](){ loop += 1; change = pips_sum = pips_change = 0; return false; }()){ mpre("ОШИБКА скидывания флага изменений", __LINE__);
											...
											}else if([&](){ random = (ARGV.end() == ARGV.find("-r") ? "1" : ARGV.at("-r")); return (0 >= random.length()); }()){ mpre("ОШИБКА аргумент перемешивания не задан -r", __LINE__);
									...
									//}else if(true){ mpre("Остановка потока выполнения Do", __LINE__);
									}else if([&](){ // Расчет каждого из входных значений
										TMMi _BMF_INDEX; TMs _index, dano, itog; int count = 0, key;
		...
		return false; }()){ mpre("ОШИБКА создания рабочих функций", __LINE__);
			...
			static std::vector<std::thread> threads; int thread;
			if(0 >= in.size()){ mpre("Данные для обучения не заданы", __LINE__);
			//}else if([&](){ thread = (ARGV.end() == ARGV.find("-t") ? threads_count : stoi("" == ARGV.at("-t") ? "0" : ARGV.at("-t"))); return false; }()){ mpre("ОШИБКА получения количества потоков", __LINE__);a
			}else if([&](){ // Расчет количества потоков
					string _thread;
					if(0 <= (thread = -1)){ mpre("ОШИБКА установленки первоначального значение количества потоков", __LINE__);
					}else if([&](){ _thread = (ARGV.end() == ARGV.find("-t") ? "0" : ARGV.at("-t")); return (0 >= _thread.length()); }()){ mpre("Количество потоков не задано в консоли", __LINE__);
					}else if(string::npos != _thread.find_first_not_of("0123456789")){ mpre("ОШИБКА формат количества потоков не число "+ _thread, __LINE__);
					}else if([&](){ thread = stoi(_thread); return false; }()){ mpre("ОШИБКА конвертации количества потоков в число", __LINE__);
					}else{ //mpre("Количество потоков "+ _thread, __LINE__);
					}
				return (0 > thread); }()){ mpre("ОШИБКА расчета количества потоков", __LINE__);
			}else if([&](){ for(int _thread = 1; _thread <= thread; _thread++){
					std::thread th;
					if([&](){ th = std::thread(Do, thread, _thread); return false; }()){ mpre("ОШИБКА запуска потока", __LINE__);
					}else if([&](){ threads.push_back(std::move(th)); return (0 >= threads.size()); }()){ mpre("ОШИБКА добавления потока в справочник", __LINE__);
					...
					}else if([&](){ th.join(); return false; }()){ mpre("ОШИБКА отключения от потока", __LINE__);
		...
		return false; }()){ mpre("ОШИБКА перебора всех входных значений", __LINE__);
			...
			Value = ([&](TMs itog_values){
				...
				//}else if(true){ mpre("ОШИБКА Расчет временно остановлен из за функции Calc", __LINE__);
						...
						}else if([&](){ index["val"] = Learn(index, "", BMF_DANO_EX, BMF_ITOG_EX);	return index.empty(); }()){ mpre("ОШИБКА расчета значения морфа", __LINE__);
		...
		return false; }()){ mpre("ОШИБКА создания функции расчета итога", __LINE__);
			...
			nlohmann::json j, _in; json el; TMs dano;
			if(ARGV.end() == ARGV.find("-j")){ //mpre("Данные для расчета не указаны", __LINE__);
			}else if([&](){ _in = ("" == ARGV.at("-j") ? in : json::parse(ARGV.at("-j"))); return false; }()){ mpre("ОШИБКА разбора строки атрибута -j", __LINE__);
			//}else if([&](){ j["status"] = {{"size", in.size()}, {"change", change_sum}, {"pips", pips_perc}, {"bmf", BMF_INDEX_EX.at("").size()}}; return j.empty(); }()){ mpre("ОШИБКА добавления статистики", __LINE__);
			}else if(0 < change_sum){ mpre("При ошибках результат не выводим", __LINE__);
			}else if([&](){ for(auto& js:_in.items()){ // Расчет списка теста
					TMs dano, itog; json el; string key; int cache;
					if([&](){ el = js.value(); return el.empty(); }()){ mpre("ОШИБКА элемент не найден", __LINE__);
					}else if([&](){ key = js.key(); return (0 >= key.length()); }()){ mpre("ОШИБКА расчета ключа", __LINE__);
					}else if(el.end() == el.find("dano")){ mpre("ОШИБКА исходные данные не могут быть пустыми", __LINE__);
					}else if([&](){ TMs _dano = el.at("dano"); dano = _dano; return dano.empty(); }()){ mpre("ОШИБКА получения входных знаков", __LINE__);
					}else if([&](){ // Получение итога
						...
						return false; }()){ mpre("ОШИБКА получения итога", __LINE__);
					}else if([&](){ mpre("РЕЗУЛЬТАТ: "+ el.dump(), __LINE__); return false; }()){ mpre("ОШИБКА уведомления", __LINE__);
					}else if([&](){ cache = Crc32(el["dano"].dump().c_str()); return false; }()){ mpre("ОШИБКА расчета ключа кеша", __LINE__);
					}else if([&](){ Vals(dano, "dano", cache, BMF_DANO_EX, BMF_DANO_VALUES_EX, BMF_DANO_TITLES_EX, clump_id); return BMF_DANO_EX.empty(); }()){ mpre("ОШИБКА установки входящих значений", __LINE__);
							...
							}else if([&](){ itog_values = Value(itog_values); return itog_values.empty(); }()){ mpre("ОШИБКА расчета значения", __LINE__);
							...
							if([&](){ itog_values = itog_values_itr.second; return itog_values.empty(); }()){ mpre("ОШИБКА получения значения итога", __LINE__);
								...
								if([&](){ itog_titles = erb(BMF_ITOG_TITLES_EX, {{"itog_values_id", itog_values["id"]}, {"value", value}}); return itog_titles.empty(); }()){ //mpre("Заголовок значения `"+ itog_values["name"]+ "` не найден", __LINE__);
								}else if(itog_titles.end() == itog_titles.find("name")){ mpre("ОШИБКА поле `name` не найдено у заголовка", __LINE__);
								}else if([&](){ j[key][name] = itog_titles.at("name"); return false; }()){ mpre("ОШИБКА установки заголовка значению", __LINE__);
				...
				} return false; }()){ mpre("ОШИБКА расчета списка теста", __LINE__);
		...
		return false; }()){ mpre("ОШИБКС создания функции сохранения в базу данных", __LINE__);
			...
			if(in.empty()){ mpre("Пустой массив обучающей выборки. Не сохраняем данные в БД", __LINE__);
				...
				return false; }()){ mpre("ОШИБКА установки прав досутпа к файлу", __LINE__);
					...
					TMs test; string shift, date, args; nlohmann::json arg;
					//if((in.size() == 1) && (0 >= change_sum)){ mpre("Один пример без изменений не сохраняем историю", __LINE__);
					...
					}else if([&](){ arg = ARGV; arg.erase("-"); arg.erase("-j"); return arg.empty(); }()){ mpre("ОШИБКА получения строки аргументов", __LINE__);
					}else if([&](){ test = fk("mp_bmf_test", {}, {{"time", to_string(timestamp)}, {"date", date}, {"size", to_string(in.size())}, {"change", to_string(change_sum)}, {"duration", to_string(time(0)-timestamp)}, {"clump", clump_id}, {"loop", to_string(loop)}, {"perc", to_string(perc)}, {"pips", to_string(pips_perc)}, {"bmf", to_string(BMF_INDEX_EX.at("").size())}, {"args", arg.dump()}}, {}); return false; }()){ mpre("ОШИБКА сохранения результатов тестов", __LINE__);
			...
			}else if([&](){ // Сохранение данных в базу для получения идентификаторов
					//if(0 == index_save){ mpre("Сохранение морфов отключено "+ to_string(BMF_INDEX_EX.at("").size())+ " "+ to_string(loop), __LINE__);
					...
					if([&](){ itog = _itog = itog_itr.second; return itog.empty(); }()){ mpre("ОШИБКА получения морфа из итератора", __LINE__);
						...
						return false; }()){ mpre("ОШИБКА установка ссылки на морф", __LINE__);
							...
							}else if([&](){ _itog_values = _BMF_ITOG_VALUES.at(stoi(itog.at("itog_values_id"))); return _itog_values.empty(); }()){ mpre("ОШИБКА выборки морфа по первой связи", __LINE__);
					...
					if([&](){ dano = _dano = dano_itr.second; return dano.empty(); }()){ mpre("ОШИБКА получения морфа из итератора", __LINE__);
							...
							}else if([&](){ _dano_values = _BMF_DANO_VALUES.at(stoi(dano.at("dano_values_id"))); return _dano_values.empty(); }()){ mpre("ОШИБКА выборки морфа по первой связи", __LINE__);
					...
					if([&](){ _index = index = index_itr.second; return index.empty(); }()){ mpre("ОШИБКА получения морфа из итератора", __LINE__);
							...
							}else if([&](){ index.at("itog_id") = itog.at("id"); return false; }()){ mpre("ОШИБКА установки значения связи", __LINE__);
							}else{ //mpre(index, __LINE__, "Измененная связь");
							} return index.empty();
						}()){ mpre("ОШИБКА установки старшего морфа", __LINE__);
					...
					if([&](){ _dano_titles = dano_titles = dano_titles_itr.second; return dano_titles.empty(); }()){ mpre("ОШИБКА получения заголовка", __LINE__);
							...
							}else if([&](){ _dano_values = _BMF_DANO_VALUES.at(stoi(dano_titles.at("dano_values_id"))); return _dano_values.empty(); }()){ mpre("ОШИБКА выборки морфа по первой связи", __LINE__);
							}else if(_dano_values.end() == _dano_values.find("id")){ mpre("ОШИБКА получения идентификатора связи", __LINE__);
							}else if([&](){ dano_titles.at("dano_values_id") = _dano_values.at("id"); return false; }()){ mpre("ОШИБКА установки значения связи", __LINE__);
							}else{ //mpre("Установка ссылки на морф", __LINE__);
							}
						return false; }()){ mpre("ОШИБКА установка ссылки на морф", __LINE__);
					...
					if([&](){ _itog_titles = itog_titles = itog_titles_itr.second; return itog_titles.empty(); }()){ mpre("ОШИБКА получения заголовка", __LINE__);
							...
							}else if([&](){ _itog_values = _BMF_ITOG_VALUES.at(stoi(itog_titles.at("itog_values_id"))); return _itog_values.empty(); }()){ mpre("ОШИБКА выборки морфа по первой связи", __LINE__);
							}else if(_itog_values.end() == _itog_values.find("id")){ mpre("ОШИБКА получения идентификатора связи", __LINE__);
