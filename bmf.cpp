...
int main(int argc, char **argv){
	if([&](){ // Список значений из консоли
		...
		}else if([&](){ in = json::parse(bmf::ARGV.at("-")); return false; }()){ mpre("ОШИБКА Входящий параметр с обучающими данными пуст", __LINE__);
		...
		if([&](){ // Отображение атрибутов командной строки
			...
			if(string file = (1 <= argc ? argv[0] : "./bimorph"); (0 >= file.length())){ mpre("ОШИБКА получения имени файла", __LINE__);
				...
				//std::cout << "-split	делитель итога используется для распределения расчетов" << endl;
		...
		} return dbname; }(); false){ mpre("ОШИБКА получения имени базы данных", __LINE__);
	}else if([&](){ // Подключение базы
		if(bmf::exec = ([&](string sql){ // Запрос к БД
			...
			if([&](){
				...
				} return false; }()){ mpre("ОШИБКА получения скопления", __LINE__);
				...
				if(bmf::exec("CREATE TABLE IF NOT EXISTS main.mp_bmf_index (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,`clump_id` INTEGER,`itog_values_id` INTEGER, `depth` INTEGER, `dimension` INTEGER,`dano_id` INTEGER,`itog_id` TEXT,`calc_pos_id` INTEGER,`index_id` INTEGER, `bmf-index` INTEGER)"); false){ mpre("ОШИБКА создания таблицы морфов", __LINE__);
				}else if(bmf::exec("CREATE TABLE IF NOT EXISTS main.mp_bmf_dano (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,`time` INTEGER,`uid` INTEGER,`clump_id` INTEGER,`dano_values_id` INTEGER,`name` TEXT,`val` INTEGER,`values` TEXT)"); false){ mpre("ОШИБКА создания таблицы морфов", __LINE__);
				...
				}else if(bmf::exec("CREATE TABLE IF NOT EXISTS main.mp_bmf_itog (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,`time` INTEGER,`clump_id` INTEGER,`index_id` INTEGER,`itog_values_id` INTEGER,`name` TEXT,`val` INTEGER,`values` TEXT)"); false){ mpre("ОШИБКА создания таблицы морфов", __LINE__);
				...
				}else if(bmf::exec("CREATE TABLE IF NOT EXISTS main.mp_bmf_itog_values (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,`time` INTEGER,`index_type_id` INTEGER,`clump_id` INTEGER,`name` TEXT,`value` INTEGER,`itog_values_option_id` INTEGER,`bin` INTEGER)"); false){ mpre("ОШИБКА создания значения итога", __LINE__);
				}else if(bmf::exec("CREATE TABLE IF NOT EXISTS main.mp_bmf_itog_titles (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,`time` INTEGER, `clump_id` INTEGER, `itog_values_id` INTEGER, `value` INTEGER, `name` INTEGER, `text` TEXT)"); false){ mpre("ОШИБКА создания значения итога", __LINE__);
				}else if(bmf::exec("CREATE TABLE IF NOT EXISTS main.mp_bmf_dano_titles (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,`time` INTEGER, `clump_id` INTEGER, `dano_values_id` INTEGER, `value` INTEGER, `name` INTEGER, `text` TEXT)"); false){ mpre("ОШИБКА создания значения итога", __LINE__);
				}else{ return false;
			...
			}else if([&](){ BMF_CALC = bmf::Dataset(BMF_CALC, "BMF_CALC", "mp_bmf_calc", "Расчеты"); return BMF_CALC_POS.empty(); }()){ mpre("ОШИБКА выборки позиций расчетов", __LINE__);
			}else if([&](){ BMF_CALC_ORIENT = bmf::Dataset(BMF_CALC_ORIENT, "BMF_CALC_ORIENT", "mp_bmf_calc_orient", "Направления"); return BMF_CALC_ORIENT.empty(); }()){ mpre("ОШИБКА выборки направлений расчетов", __LINE__);
			}else if([&](){ BMF_CALC_POS = bmf::Dataset(BMF_CALC_POS, "BMF_CALC_POS", "mp_bmf_calc_pos", "Позиции"); return BMF_CALC_POS.empty(); }()){ mpre("ОШИБКА выборки позиций расчетов", __LINE__);
			}else if([&](){ BMF_CALC_VAL = bmf::Dataset(BMF_CALC_VAL, "BMF_CALC_VAL", "mp_bmf_calc_val", "Знаки"); return BMF_CALC_VAL.empty(); }()){ mpre("ОШИБКА выборки значений расчетов", __LINE__);
			}else if([&](){ BMF_INDEX_TYPE = bmf::Dataset(BMF_INDEX_TYPE, "BMF_INDEX_TYPE", "mp_bmf_index_type", "Тип значения"); return BMF_INDEX_TYPE.empty(); }()){ mpre("ОШИБКА выборки типа значений", __LINE__);
			}else if([&](){ TMMi DANO = bmf::Tab("SELECT * FROM `mp_bmf_dano` -- WHERE `clump_id`='"+ bmf::clump_id+ "'"); BMF_DANO_EX[""] = DANO; return false; }()){ mpre("ОШИБКА получения дано скопления", __LINE__);
			}else if([&](){ TMMi DANO_VALUES = bmf::Tab("SELECT * FROM `mp_bmf_dano_values` -- WHERE `clump_id`='"+ bmf::clump_id+ "'"); BMF_DANO_VALUES_EX[""] = DANO_VALUES; return false; }()){ mpre("ОШИБКА получения дано значений", __LINE__);
			}else if([&](){ TMMi DANO_TITLES = bmf::Tab("SELECT * FROM `mp_bmf_dano_titles` -- WHERE `clump_id`='"+ bmf::clump_id+ "'"); BMF_DANO_TITLES_EX[""] = DANO_TITLES; return false; }()){ mpre("ОШИБКА получения дано справочника", __LINE__);
			}else if([&](){ TMMi ITOG_VALUES = bmf::Tab("SELECT * FROM `mp_bmf_itog_values` -- WHERE `clump_id`='"+ bmf::clump_id+ "'"); BMF_ITOG_VALUES_EX[""] = ITOG_VALUES; return false; }()){ mpre("ОШИБКА получения итогов значений", __LINE__);
			}else if([&](){ TMMi ITOG_TITLES = bmf::Tab("SELECT * FROM `mp_bmf_itog_titles` -- WHERE `clump_id`='"+ bmf::clump_id+ "'"); BMF_ITOG_TITLES_EX[""] = ITOG_TITLES; return false; }()){ mpre("ОШИБКА получения итогов справочника", __LINE__);
			}else if([&](){ TMMi ITOG = bmf::Tab("SELECT * FROM `mp_bmf_itog` -- WHERE `clump_id`='"+ bmf::clump_id+ "'"); BMF_ITOG_EX[""] = ITOG; return false; }()){ mpre("ОШИБКА получения итогов скопления", __LINE__);
				...
				return false; }()){ mpre("ОШИБКА получения списка итогов", __LINE__);
					...
					string sql;
					if([&](){ sql = "SELECT * FROM `mp_bmf_index` WHERE "+ (((bmf::ARGV.end() == bmf::ARGV.find("-itog"))) ? "1=1" : "itog_id='"+ bmf::ARGV.at("-itog")+ "'"); return (0 >= sql.length()); }()){ mpre("ОШИБКА составления запроса на выборку морфов", __LINE__);
					}else if([&](){ BMF_INDEX_EX[""] = bmf::Tab(sql); return false; }()){ mpre("ОШИБКА выборки списка морфов из базы", __LINE__);
		...
		}else if(bmf::Choice = ([&](TMs index, TMs stairs, TM3i& _BMF_DANO_EX, TM3i& _BMF_ITOG_EX, TMs _dano = {}){ //mpre("Выбор итога для расширения", __LINE__);
		...
		if(false){ mpre("ОШИБКА уведомления", __LINE__);
			...
			if(false){ mpre("Пропуск установки значений", __LINE__);
							...
							for(int i = 0; i < _value_.length(); i++){ // Добавление знаков значению
								if(string val = _value_.substr(i, 1); (1 != val.length())){ mpre("ОШИБКА получения символа "+ to_string(i), __LINE__);
									...
									//}else if(bmf::ARGV.end() != bmf::ARGV.find("-t")){ mpre(values, __LINE__, "Значение"); mpre("ОШИБКА в многопоточном режиме недопустимо создание новых знаков "+ alias+ "_values_id="+ values.at("id")+ " name="+ to_string(nn), __LINE__);
										...
										}else if(string _itog = bmf::ARGV.at("-itog"); "0" != _itog){ itog = _itog;
										}else if(std::string table = "mp_bmf_"+ alias; table.empty()){ mpre("ОШИБКА расчета имени таблицы", __LINE__);
										}else if(vals = bmf::fk(table, {}, vals, {}); vals.empty()){ mpre(vals, __LINE__, "Знак"); mpre("ОШИБКА добавления знака", __LINE__);
										}else{ mpre("Добавление знака " +alias +" " +vals.at("id"), __LINE__);
									...
									}else{ //mpre(vals, "Знак", __LINE__); mpre("Добавление глобального знака", __LINE__); //mpre("ОШИБКА добавление глобального знака", __LINE__); //mpre(*BMF, __LINE__, "Список знаков"); mpre("Добавление глобального знака `"+ values["name"]+ "` ["+ vals.at("id")+ "] "+ to_string(nn), __LINE__); mpre(vals, __LINE__, "Знак"); //mpre("ОШИБКА добавления знака "+ alias+ " "+ vals.at("id"), __LINE__);
									...
									}else if(_vals = vals; _vals.empty()){ mpre("ОШИБКА копирования глобального значения в локальное", __LINE__); //mpre(_vals, __LINE__, "Знак `"+ values["name"]+ "` уже добавлен "+ to_string(nn));
				...
				if(bool cache = false; !cache){ //mpre("Кеширование значений отключено " +to_string(key), __LINE__);
					...
					if(DANO.at(alias).end() != DANO.at(alias).find(key)){ //mpre("Структура знаков уже создана", __LINE__);
						...
						}else if(string val = vals.at("val"); (1 != val.length())){ mpre(vals, __LINE__, "Знак"); mpre("ОШИБКА выборки значения знака", __LINE__);
		...
		}else if(bmf::Learn = ([&](TMs bmf_index, string val = "", int key, TM3i& _BMF_DANO_EX, TM3i& _BMF_ITOG_EX, TMs stair = {}){ //mpre("Обучение "+ index.at("id"), __LINE__);  //system("sleep 1"); // mpre(BMF_INDEX, __LINE__, "Список"); // if(remove){ mpre(row, __LINE__, "Ученик"); }
			...
			//}else if(boost::dynamic_bitset<unsigned char> gmap = boost::dynamic_bitset<unsigned char>{vmap}; false){ mpre("ОШИБКА установка битовой маски ", __LINE__);
					...
					if(STAIRS.empty()){ mpre("ОШИБКА лестница пуста", __LINE__);
						...
						if(index_1.empty()){ val_1 = dano.at("val");
						}else if(bmf::CACHE.end() == bmf::CACHE.find(index_1.at("id"))){ mpre(bmf::CACHE, "Кеш", __LINE__); mpre(index, "Морф", __LINE__); mpre("ОШИБКА кеш не найден " +index_1.at("id"), __LINE__);
						}else if(val_1 = bmf::CACHE.at(index_1.at("id")); (1 != val_1.length())){ mpre("ОШИБКА выборки кеша старшей связи", __LINE__);
						...
						if(index_0.empty()){ val_0 = dano.at("val");
						}else if(bmf::CACHE.end() == bmf::CACHE.find(index_0.at("id"))){ mpre(bmf::CACHE, "Кеш", __LINE__); mpre("ОШИБКА кеш не найден " +index_0.at("id"), __LINE__);
						}else if(val_0 = bmf::CACHE.at(index_0.at("id")); (1 != val_0.length())){ mpre("ОШИБКА выборки кеша младшей связи", __LINE__);
						}else{ //mpre("ОШИБКА значение не найдено", __LINE__);
						...
						if(int calc_pos_id = atoi(index["calc_pos_id"].c_str()); (0 > calc_pos_id)){ mpre("ОШИБКА формата позисии морфа", __LINE__);
						...
						}else if(bmf::CACHE[index.at("id")] = stairs_itr->second["val"] = stairs["val"] = val; (1 != stairs["val"].length())){ mpre("ОШИБКА расчета значения", __LINE__);
						}else{ //mpre("Расчет значения status=" +stairs.at("status") +" "+ index.at("id") +" calc_pos_id=" +std::to_string(calc_pos_id) + " val_1=" +val_1 +" val_0=" +val_0 +" val=" +val, __LINE__);
						...
						}else{ //mpre(transfer, "Выбор направления status=" +stairs.at("status") +" " +index.at("id"), __LINE__);
						...
						if(string::npos != stairs.at("status").find_last_not_of("12")){ //mpre("Расчет направления только для первого второго статуса", __LINE__);
							...
							}else if("" == transfer.at("other_id")){ _dano = bmf::Choice(index, stairs, _BMF_DANO_EX, _BMF_ITOG_EX);
						...
						}else if(_index = {{"id", bmf::Id(BMF_INDEX_EX.at(""))}, {"clump_id", index.at("clump_id")}, {"calc_pos_id", calc_pos_id}, {"itog_values_id", index.at("itog_values_id")}, {"depth", depth}, {"dimension", "1"}, {"dano_id", _dano.at("id")}, {"itog_id", index.at("itog_id")}, {"index_id", ""}, {"bmf-index", ""}}; _index.empty()){ mpre("ОШИБКА создания нового морфа "+ index.at("id"), __LINE__);
						...
						}else{ //mpre("Добавление нового морфа status=" +stairs.at("status") +" " +index.at("id") +" > "+ _index.at("id") +" _val=" +_val, __LINE__); //mpre(index, "Морф", __LINE__); mpre(stairs, "Ступень", __LINE__);
			...
			//}else if(true){ err("Остановка расчета");
				...
				if(string _val = bmf::Learn(index, itog.at("val"), key, _BMF_DANO_EX, _BMF_ITOG_EX); (1 <= _val.length() >= 2)){ mpre("ОШИБКА обучения морфа значение не верная длинна результата", __LINE__);
		...
		}else if(bmf::LearningAll = ([&](json js, int thread, int _thread, int key, TM3i& _BMF_DANO_EX, TM3i& _BMF_ITOG_EX, int change = 0){ //mpre("ОбновлениеОбщее", __LINE__); // Обучение
			...
			}else if(TM3i _BMF_DANO_EX = BMF_DANO_EX; false){ mpre("ОШИБКА установки первоначальных исходников дано", __LINE__);
			}else if(TM3i _BMF_ITOG_EX = BMF_ITOG_EX; false){ mpre("ОШИБКА установки первоначальных итогов дано", __LINE__);
			}else if(int server = [&](std::string srv = ""){ // Запуск сервера
				if(bmf::ARGV.end() == bmf::ARGV.find("-server")){ //mpre("Сервер в параметрах не указан", __LINE__);
				}else if(int s = socket(AF_INET, SOCK_STREAM, 0); s<0){ err("Ошибка установки сокета");
				}else if(struct sockaddr_in addr; false){ err("Структура соединения");
				}else if(addr.sin_family = AF_INET; false){ err("Тип соединения");
				}else if(addr.sin_port = htons(18666); false){ err("Порт соединения");
				}else if(mpre("Ожидание соединения 127.0.0.1:18666", __LINE__); false){ err("Уведомление");
				}else if(addr.sin_addr.s_addr = htonl(INADDR_ANY); false){ err("Адрес соединения");
				}else if(bind(s, (struct sockaddr *)&addr, sizeof(addr)) < 0){ err("Установка прослушивания");
				}else if([&](int loop = 2){ do{ // Получение соединения от клиентов
					if(listen(s, 5)){ err("Отметка порта как пассивный - он будет слушать порт");
					}else if(int s1 = accept(s, NULL, NULL); s1 < 0){ err("Прием соединения");
					}else if(char buffer[1024]; false){ err("Данные из порта");
					}else if(int counter = 0; false){ err("Счетчик");
					}else if(memset(buffer, 0, sizeof(buffer)); false){ err("Сохранение в память");
					}else if(int rc = recv(s1, buffer, 30, 0); rc == 0){ mpre("Отключен", __LINE__);
					}else if(rc < 0){ err("Системная"); //if( errno == EINTR ){ continue; }
					}else if(char response[] = "Hi, dear!"; false){ err("Текст ответа");
					}else if(sendto( s1, response, sizeof(response), 0, (struct sockaddr *)&addr, sizeof(addr) ) < 0){ err("Ответ клиенту");
					//}else if(char *ip = inet_ntoa(addr.sin_addr); false){ err("Получение удаленного адреса");
					}else{ printf("%s\n", buffer);
					}}while(--loop>0); return false; }()){ err("Ожидание ответа");
				}else{ //mpre("Ответ отправлен `" +string(response) + "`", __LINE__); //cout<<"\n Сообщение от клиента : "<<buffer<<endl; //mpre("Сервер " +server, __LINE__);
				} return 0; }(); false){ mpre("Остановка сервера ", __LINE__);
			}else if(int client = [&](std::string clt = ""){ // Запуск сервера
				if(bmf::ARGV.end() == bmf::ARGV.find("-client")){ //mpre("Сервер в параметрах не указан", __LINE__);
				}else if(int s = socket( AF_INET, SOCK_STREAM, 0 ); s<0){ mpre("ОШИБКА обновления сокета", __LINE__);
				}else if(struct sockaddr_in peer; false){ mpre("ОШИБКА создания структуры соединения", __LINE__);
				}else if(peer.sin_family = AF_INET; false){ err("Установка типа соединения");
				}else if(peer.sin_port = htons(18666); false){ err("Установка порта");
				}else if(peer.sin_addr.s_addr = inet_addr("192.168.1.6"); false){ err("Установка адреса");
				}else if(int result = connect( s, ( struct sockaddr * )&peer, sizeof( peer ) ); result){ err("Соединение с сервером");
				}else if(string buf = "Hello, world! " +to_string(time(0)); false){ err("Данные для отправки");
				}else if(result = send(s, buf.c_str(), buf.length(), 0); result <= 0){ err("Отправка данных");
				}else if(shutdown(s, 1) < 0){ err("Error calling shutdown");
				}else if(fd_set allreads = [&](fd_set allreads = {}){ FD_ZERO( &allreads ); FD_SET( 0, &allreads ); FD_SET( s, &allreads ); return allreads; }(); false){ err("Переменная соединений");
				}else if([&](){ do{ // Ожидание ответа
					if(fd_set readmask = allreads; false){ err("Первоначальное соединение");
					}else if(select(s + 1, &readmask, NULL, NULL, NULL ) <= 0){ err("ОШИБКА выбора");
					}else if(FD_ISSET( 0, &readmask )){ err("Нет ответа сервера");
					}else if(!FD_ISSET( s, &readmask )){ err("Неизвестная ошибка");
					}else if(char buffer[20]; false){ err("Буфер ответа");
					}else if(memset(buffer, 0, 20*sizeof(char)); false){ err("Загрузка в память");
					}else if(int result = recv( s, buffer, sizeof(buffer) - 1, 0 ); (0 == result)){ mpre("Отключение сервера", __LINE__);
					}else if(result < 0){ err("Ответ сервера");
					}else{ mpre("Ответ сервера: " +string(buffer), __LINE__);
					} return 0; }while(false); }(); false){ err("Ответ");
				}else{ //mpre("Соединение установлено", __LINE__);
				} return 0; }(); false){ mpre("Остановка клиента "+ client, __LINE__);
			}else if(auto _in = in; _in.empty()){ mpre("ОШИБКА выборки массива данных", __LINE__);
					...
					do{ // Расчет и обучение входящих параметров
						if(change = 0; false){ mpre("ОШИБКА скидывания флага изменений", __LINE__);
							...
							}else if(string::npos != random.find_last_not_of("01")){ mpre("ОШИБКА формат перемешивания задан неправильно "+ random, __LINE__);
		...
		if(int thread = [&](int thread = -1){ // Расчет количества потоков
		...
		}else if(nlohmann::json _in = ("" == bmf::ARGV.at("-dano") ? in : json::parse(bmf::ARGV.at("-dano"))); false){ mpre("ОШИБКА разбора строки атрибута -dano", __LINE__);
		...
		//}else if(std::experimental::filesystem::perms p = std::experimental::filesystem::status(dbname).permissions(); ((p & std::experimental::filesystem::perms::others_write) == std::experimental::filesystem::perms::none)){ mpre("ОШИБКА файл БД не доступен для записи $chmod u+w "+ dbname, __LINE__);
			...
			if(bmf::exec("CREATE TABLE IF NOT EXISTS mp_bmf_test (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,`time` INTEGER, `date` TEXT, `size` INTEGER, `args` TEXT, `change` INTEGER, `duration` INTEGER, `bmf` INTEGER, `loop` INTEGER, `perc` REAL, `pips` REAL, `clump` TEXT)"); false){ mpre("ОШИБКА создания значения итога", __LINE__);
				...
				if(TMs index = index_itr.second; index.empty()){ mpre("ОШИБКА получения морфа из итератора", __LINE__);
					...
					}else if(_BMF_INDEX.end() == _BMF_INDEX.find(atoi(index_id.c_str()))){ mpre("ОШИБКА морф в списке не найден "+ index_id, __LINE__);
