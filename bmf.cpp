...
//	std::function<Db*(string)> Open; // Список баз данных
	...
	std::function<string(string,int)> Table_grp; // РРасчет группы
	std::function<bool(string,string,int,string,bool,string,boost::dynamic_bitset<>&,boost::dynamic_bitset<>&,TMMs&,TMMs*,int)> Val; // Значение дочерних групп
	//}else if(bmf::Val = [&](string addr ,string itog_id ,int key ,string table_grp ,bool dv ,string ig0_id ,boost::dynamic_bitset<> &exist, boost::dynamic_bitset<> &vals ,TMMs &BMF_INDEX ,TMMs *GRP , int line){ // Расчет значения
}
...
int main(int argc, char **argv){
	if(setlocale(LC_ALL, "LC_ALL=C.UTF-8"); false){ mpre("ОШИБКА установки кирилицы", __LINE__);
		...
		if([&](){ // Очистка БД
			...
			if(bmf::ARGV.end() == bmf::ARGV.find("-c")){ //mpre("Не очищаем БД", __LINE__);
				...
				if(string sql = "SHOW TABLES FROM " +bmf::dbname +";"; sql.empty()){ err("Запрос списка всех таблиц базы данных");
					...
					}else if(string::npos == table.find("index_grp_")){ //mpre("", __LINE__); mpre("Неизвестная таблица `" +table +"` пропуск автоматического удаления",__LINE__);
			...
			if("mysql" != bmf::dbtype){ //mpre("Используем не mysql", __LINE__);
			}else if(mysql_query(bmf::mysql, std::string("CREATE TABLE IF NOT EXISTS `index` (`id` INT AUTO_INCREMENT NOT NULL PRIMARY KEY, grp INT NOT NULL ,dataset_id INT NOT NULL ,`key` INT NOT NULL ,itog_id INT NOT NULL , addr TEXT ,md5 VARCHAR(32) NOT NULL ,`timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, `dano_id` INT NOT NULL, UNIQUE KEY index_md5(md5), KEY `itog_md5`(itog_id ,md5) ,KEY timestamp (timestamp))").c_str())){ err("Список морфов"); 
			}else if(mysql_query(bmf::mysql, std::string("CREATE TABLE IF NOT EXISTS `index_grp` (`id` INT AUTO_INCREMENT NOT NULL PRIMARY KEY ,index_grp_id INT DEFAULT NULL ,itog_id INT NOT NULL ,dataset_id INT NOT NULL ,`index_id` INT NOT NULL ,count INT(1) DEFAULT NULL ,`key` INT NOT NULL ,list TEXT ,`grow` INT DEFAULT NULL, vals BIT(64) DEFAULT NULL ,exist BIT(64) DEFAULT NULL ,dano BIT(64) DEFAULT NULL, UNIQUE KEY `index_vals_unique` (dataset_id ,itog_id ,`key` ,`index_id`) ,KEY exist(exist) ,FOREIGN KEY (index_grp_id) REFERENCES index_grp(id))").c_str())){ mpre("ОШИБКА Вставка данных " +string(mysql_error(bmf::mysql)) ,__LINE__); 
			...
			}else if(mysql_query(bmf::mysql, std::string("CREATE TABLE IF NOT EXISTS `itog` (`id` INT AUTO_INCREMENT NOT NULL PRIMARY KEY ,`itog_values_id` INT, `name` TEXT, `val` VARCHAR(750), `index_id` INT)").c_str())){ err("Итог"); 
			}else if(mysql_query(bmf::mysql, std::string("CREATE TABLE IF NOT EXISTS `itog_values` (`id` INT AUTO_INCREMENT NOT NULL PRIMARY KEY, `name` TEXT)").c_str())){ err("Итоги значения"); 
			}else if(mysql_query(bmf::mysql, std::string("CREATE TABLE IF NOT EXISTS `itog_titles` (`id` INT AUTO_INCREMENT NOT NULL PRIMARY KEY, `itog_values_id` INT NOT NULL, `name` TEXT, `value` TEXT , UNIQUE KEY `name_unique` (itog_values_id ,name))").c_str())){ err("Итоги заголовки"); 
		...
		if(bmf::Choice = ([&](std::string list ,TMs index ,string link, int key ,string itog_id ,TMMs &DANO, TMMb &DANO_BITMAP ,int line){ //mpre("Выбор итога для расширения", __LINE__);
			...
			if(DANO_BITMAP.empty()){ err("Карта бит пуста");
			...
			}else if(auto [dano_id ,res] = [&](std::pair<string,int> pair = make_pair("" ,0)){ // Первоначальный исходник
				if(!bmf_dano_id.empty()){ //mpre("Не первоначальная связь" ,__LINE__);
				}else if([&](){ for(auto [_dano_id ,dano]:DANO){ // Уровни разнообразия
					if(auto dano_bitmap_itr = DANO_BITMAP.find(_dano_id); (DANO_BITMAP.end() == dano_bitmap_itr)){ err("Список значений исходника не найден");
					}else if(int size = dano_bitmap_itr->second.size(); !size){ err("Количество примеров в наборе");
					}else if(int count = dano_bitmap_itr->second.count(); false){ err("Количество установленных бит исходника");
					}else if(int _res = (size > count *2 ? count : size-count) ; false){ err("Расчет результата");
					}else if(pair = [&](std::pair<string,int> _pair){ // Обновленное значение пары
						if(auto [dano_id ,res] = pair; dano_id.empty()){ //mpre("Первоначальная пара не создана" ,__LINE__);
						}else if(res < _res){ //mpre("Новое значение больше старого res=" +to_string(res) +" _res=" +to_string(_res) +" " ,__LINE__);
						}else if(_pair = pair ;true){ //mpre("Сохранение старого значения res=" +to_string(res) +" _res=" +to_string(_res) +" " ,__LINE__);
						}return _pair; }(make_pair(_dano_id ,_res)) ;false){ err("Обновленное значение пары");
					}else{ //mpre("Сравнение изменений исходников" ,__LINE__);
					}}return false; }()){ err("Установка уровня разнообразия");
				}else{ //mpre("Выбор наиболее подходящего по изменениям исходника" ,__LINE__);
				}return pair; }(); false){ err("Первоначальный исходник");
				...
				if(!dano_id.empty()){ //mpre("Выбран первоначальный исходник" ,__LINE__);
			...
			}else if(TM3i RES = [&](TM3i RES = {}){ // Список подходящих связей
				if(!dano_id.empty()){ //mpre("Уже выбран первоначальный исходник dano_id=" +dano_id ,__LINE__);
				}else if(bmf_dano_id.empty()){ err("Значение вышестоящего исходника");
				}else if(DANO_BITMAP.end() == DANO_BITMAP.find(bmf_dano_id)){ err("Прошлый исходник в списке биткарт не найден");
				}else if([&](){ for(auto [dano_id ,dano]:DANO){ // Соответствие
					//if(bmf_dano_id == dano_id){ //mpre("Кроме себя" ,__LINE__);
					if(DANO_BITMAP.end() == DANO_BITMAP.find(dano_id)){ err("Исходник в списке биткарт не найден");
					}else if(string val_cur = (DANO_BITMAP.at(dano_id).test(key) ? "1" : "0"); val_cur.empty()){ err("Текущее значение");
					}else if(string val_prev = (DANO_BITMAP.at(dano_id).test(_key) ? "1" : "0"); val_prev.empty()){ err("Предыдущее значение");
					}else if(val_cur == val_prev){ //mpre("Значения равны dano_id=" +dano_id ,__LINE__);
					}else if(string grp = val_prev +val_cur; grp.empty()){ err("Группа");
					}else if(bool lock = EPT.end() != EPT.find(dano_id); false){ err("Значение запрета");
					}else if(string _grp = (lock ? "lock_" : "unlock_") +grp; _grp.empty()){ err("Имя группы");
					}else if(int pos = (lock ?atoi(EPT.at(dano_id).c_str()) :(DANO_BITMAP.at(dano_id) ^DANO_BITMAP.at(bmf_dano_id)).count()) ;false){ err("Соответствие");
					}else if(RES[_grp][pos][dano_id] = ""; RES.empty()){ err("Установка значения");
					}else{ //mpre("Добавление значения к списку ресурсов" ,__LINE__);
			...
			//}else if(mpre(RES ,"Ресурсы dano_id=" +dano_id +" list=" +list, __LINE__); false){ mpre("ОШИБКА уведомления", __LINE__);
			...
			for(auto &val_itr:VALUE){ //mpre("VALUE ", __LINE__);
				if(string value = val_itr.second; false){ mpre("ОШИБКА получения значения итератора", __LINE__);
					...
					//}else if(static TMMs VALUES ;false){ err("Справочник значений");
					}else if(values = bmf::Up_mysql(table, {{"name", name}}, {}, {}, __LINE__); !values.empty()){ //mpre("Выборка значения записанного ранее" ,__LINE__);
					/*}else if([&](){ // Проверка наличия заголовка в базе
						if(auto values_itr = VALUES.find(alias) ;values_itr == VALUES.end()){ //mpre("Значение не добавлялось ранее name=" +name ,__LINE__);
						}else if(values_itr->second.end() == values_itr->second.find(name)){ //mpre("Имя не найдено в значении name=" +name ,__LINE__);
						}else if(string values_id = values_itr->second.at(name); values_id.empty()){ err("Выборка идентификатора значения");
						}else if(values = bmf::Up_mysql(table, {{"name", name}}, {}, {}, __LINE__); values.empty()){ err("Выборка значения записанного ранее");
						}else{ //mpre("Пропускаем ранее добавленный заголовок values_id=" +values_id ,__LINE__);
						}return false; }() ;!values.empty()){ //mpre("Не добавляем значение добавлен ранее name=" +name ,__LINE__);*/
					}else if(TMs _values = {{"name", name}}; _values.empty()){ mpre("ОШИБКА фонмирования нового значения", __LINE__);
					}else if((values = bmf::Up_mysql(table, _values, _values, {}, __LINE__)).empty()){ mpre("ОШИБКА обновления значения", __LINE__);
					//}else if(string values_id =values.end() ==values.find("id") ?"" :values.at("id") ;values_id.empty()){ err("Идентификатор значения");
					//}else if(VALUES[alias][name] = values_id; VALUES.empty()){ err("Добавление в справочник");
					}else{ //mpre(VALUES ,"Добавляем новое значение `"+ name+ "` ("+ alias+ ") VALUES.size()=" +to_string(VALUES.size()), __LINE__); //mpre("Добавление значения", __LINE__);
					...
					if(string name = val_itr.first; (0 >= name.length())){ mpre("ОШИБКА получения имени значения", __LINE__);
					}else if(regex b("^[0-9|.|-]+$"); regex_match(value, b)){ //mpre("Формат значения `"+ value+ "` верен ", __LINE__);
					//}else if(static TMMs TITLES ;false){ err("Статический список заголовков");
					}else if(TMs where = {{alias+ "_values_id", values["id"]} ,{"name", value}}; where.empty()){ err("Условие выборки");
					}else if(string values_id =values.end() == values.find("id") ?"" :values.at("id") ;values_id.empty()){ err("Идентификатор значения");
					}else if(bmf_titles = bmf::Up_mysql(BMF_TITLES, {{alias+ "_values_id", values_id}, {"name", value}}, {}, {}, __LINE__); !bmf_titles.empty()){ //mpre("Выборка значения записанного ранее" ,__LINE__);
					/*}else if([&](){ // Проверка наличия заголовка в базе
						if(auto titles_itr = TITLES.find(values_id) ;titles_itr == TITLES.end()){ //mpre("Значение не добавлялось ранее" ,__LINE__);
						}else if(titles_itr->second.end() == titles_itr->second.find(value)){ //mpre("Имя не найдено в значении" ,__LINE__);
						}else if(string titles_id = titles_itr->second.at(value); titles_id.empty()){ err("Выборка идентификатора заголовока");
						}else if(bmf_titles = bmf::Up_mysql(BMF_TITLES, {{alias+ "_values_id", values_id}, {"name", value}}, {}, {}, __LINE__); bmf_titles.empty()){ err("Выборка значения записанного ранее");
						}else{ //mpre("Пропускаем ранее добавленный заголовок titles_id=" +titles_id ,__LINE__);
						}return false; }() ;!bmf_titles.empty()){ //mpre("Не добавляем заголовок добавлен ранее value=" +value ,__LINE__);*/
					...
					}else if(TMs _bmf_titles = {{alias+ "_values_id", values_id}, {"name", value}, {"value", to_string(nn)}}; _bmf_titles.empty()){ mpre("ОШИБКА получения нового заголовка", __LINE__);
					}else if(bmf_titles = bmf::Up_mysql(BMF_TITLES, where, _bmf_titles, _bmf_titles, __LINE__) ;bmf_titles.empty()){ mpre("ОШИБКА обновления значения заголовка", __LINE__);
					//}else if(string titles_id =bmf_titles.end() ==bmf_titles.find("id") ?"" :bmf_titles.at("id") ;titles_id.empty()){ err("Идентификатор заголовока");
					//}else if(TITLES[values_id][value] = titles_id ;TITLES.empty()){ err("Сохранение списка заголовоков");
		...
		}else if(bmf::Table_grp = [&](string dataset_id ,int key ,string table_grp = ""){ // Имя таблицы
			if(table_grp = "index_grp" /*+string("_") +bmf::dataset.at("id")*/ +string("_") +to_string(key); table_grp.empty()){ err("Имя таблицы значений");
			}else if("index_grp" == table_grp){ //mpre("Таблица уже создана в качестве образца" ,__LINE__);
			}else if(mysql_query(bmf::mysql, std::string("CREATE TABLE IF NOT EXISTS `" +table_grp +"` LIKE `index_grp`;").c_str())){ mpre("Вставка данных " +string(mysql_error(bmf::mysql)) ,__LINE__);
			}else{ //mpre("Имя таблицы групп " +table_grp ,__LINE__);
			}return table_grp; }; false){ err("Функция расчета имени таблицы");
		}else if(bmf::Val = [&](string addr ,string itog_id ,int key ,string table_grp ,bool dv ,string ig0_id ,boost::dynamic_bitset<> &exist, boost::dynamic_bitset<> &vals ,TMMs &BMF_INDEX ,TMMs *GRP , int line){ // Расчет значения
			bool val = dv;
			if(string adr = (addr.length() %6 ? addr.substr(0 ,addr.length() %6) : addr.substr(0 ,6 -addr.length() %6)); adr.empty()){ mpre("ОШИБКА Локальный адрес addr=" +addr +" adr=" +adr ,__LINE__);
				...
				}else if(string sql = "SELECT * ,CONV(hex(exist), 16, 2) AS exist ,CONV(hex(vals), 16, 2) AS vals FROM " +table_grp +" WHERE itog_id=" +itog_id +" AND dataset_id=" +bmf::dataset.at("id") +" AND index_id=" +index_id + " AND `key`=" + to_string(key)+";"; sql.empty()){ err("Запрос на выборку значения группы");
				...
				if(nullptr == GRP){ //mpre("Список не указан __" +to_string(line) +"__" ,__LINE__);
					...
					}else if(TMMs INDEX_GRP = bmf::List_mysql("index_grp", {}, sql, __LINE__); 1 <INDEX_GRP.size()){ err("Выборка значения первоначальной группы ");
					}else if(index_grp = INDEX_GRP.begin()->second; index_grp.empty()){ err("Значение группы");
					}else{ mpre("Группа не найдена среди дочерних, делаем запрос к БД" ,__LINE__);
					}return index_grp.empty(); }()){ err("Значение группы");
				}else if([&](){ // Проверка устанвоки вышестоящей группы
					if(ig0_id.empty()){ //mpre("Не проверяем правильность установки вышестоящей группы" ,__LINE__);
		...
		if(in.empty()){ //mpre("Данные для расчета не указаны", __LINE__);
			...
			if(std::string dataset_md5 = md5(bmf::ARGV.at("-")); dataset_md5.empty()){ mpre("ОШИБКА расчета хеша набора обучающих данных", __LINE__);
			}else if(TMs _dataset = {{"count", to_string(dataset_count)}, {"md5", dataset_md5}, {"key", "0"}}; _dataset.empty()){ mpre("ОШИБКА создания записи нового набора данных", __LINE__);
			}else if((bmf::dataset = bmf::Up_mysql(bmf::DATASET, {{"md5", dataset_md5}}, _dataset, _dataset, __LINE__)).empty()){ mpre("ОШИБКА добавления набора данных", __LINE__);
			}else if(bmf::dataset.end() == bmf::dataset.find("id")){ mpre("ОШИБКА набор данных не содержит поля идентификатора", __LINE__);
			}else{ mpre("Набор данных " +bmf::dataset.at("id") +" (" +dataset_md5 +")", __LINE__);
			...
			}else{ //mpre("Итог " +dataset_map_id +" ", __LINE__); //mpre(dano_itr.second, "Исходник", __LINE__);
			}} std::cerr << endl; return false; }()){ mpre("ОШИБКА составления двоичной карты", __LINE__);
		...
		if(false){ mpre("Пропуск определения набора данных", __LINE__);
			...
			}else if(bmf::ARGV.end() == bmf::ARGV.find("learn")){ //mpre("Меняем ползунок только при обучении" ,__LINE__);
			...
			if(dataset.end() == dataset.find("count")){ mpre("ОШИБКА поле количества примеров не установлено", __LINE__);
				...
				}else if(TMs itog_map = bmf::Up(bmf::DATASET_MAP, {{"id", dataset_map_id}}, {}, {}, __LINE__); false){ mpre("ОШИБКА выборки карты исходника", __LINE__);
				}else if(string map =itog_map.end() ==itog_map.find("map") ?"" :itog_map.at("map"); false){ err("Строка карты");
				}else if(boost::dynamic_bitset<> gmap(string(dataset_count -map.length() ,'0') +map); gmap.empty()){ mpre("ОШИБКА установки карты итога", __LINE__);
		...
		//}else if(mpre(DANO ,"Исходники" ,__LINE__); false){ err("");
			...
			if(string epoch = [&](std::string epoch = "1"){ // Расчет эпохи
				...
				}else if(TMMs _INDEX = bmf::Scan_mysql(bmf::INDEX, BMF_INDEX, __LINE__); _INDEX.empty()){ //mpre("Нет изменений в структуре", __LINE__);
			...
			}else if(string table_grp = bmf::Table_grp(dataset_id ,key) ;table_grp.empty()){ // Создание таблицы значений
				...
				//}else if(mpre("Проверка", __LINE__); false){ mpre("ОШИБКА уведомления", __LINE__);
					...
					}else if(TMs dano =[&](TMs dano = {} ,float res =0){ for(auto [dano_id ,_dano]:DANO){ // Первоначальная связь
						if(dano =dano.empty() ?_dano :dano ;dano ==_dano){ //mpre("Первый попавшийся не пустой морф dano_id=" +dano_id ,__LINE__);
						}else if(DANO_BITMAP.end() == DANO_BITMAP.find(dano_id)){ err("Не задана карта сигналов исходника");
						}else if(int size = DANO_BITMAP.at(dano_id).size(); !size){ err("Количество примеров в наборе");
						}else if(int count = DANO_BITMAP.at(dano_id).count(); false){ err("Количество установленных бит исходника");
						}else if(int _res = (size > count *2 ? count : size -count) ; false){ err("Расчет результата");
						}else if(res = (_res <= res ?res :_res) ;_res != res){ //mpre("Не подходящее значение _res=" +to_string(_res) +" res=" +to_string(res) +" dataset_count=" +to_string(dataset_count) ,__LINE__);
						}else if(dano = _dano ;dano.empty()){ err("Новое значение");
						}else{ //mpre("Выбор первоначального исходника _res=" +to_string(_res) +" res=" +to_string(res) +" dataset_count=" +to_string(dataset_count) ,__LINE__);
						}}return dano; }() ;dano.empty()){ err("Выбор первоначальной связи");
					}else if(TMs _index = {{"dataset_id" ,dataset_id} ,{"key" ,to_string(key)} ,{"grp", "0"}, {"addr", "1"} ,{"md5" ,index_md5}, {"dano_id", dano.at("id")}, {"itog_id", itog.at("id")}}; _index.empty()){ mpre("ОШИБКА формирования свойст нового морфа", __LINE__);
					...
					if([&](){ // Расчетные группы
						if(!INDEX_GRP.empty()){ //mpre("Список содержит группы" ,__LINE__);
						...
						}else if(string sql = "SELECT ig0.id AS id ,ig0.id AS ig0_id ,ig1.id AS ig1_id ,ig1.itog_id AS ig0_itog_id ,ig0.index_grp_id AS index_grp_id, ig1.index_grp_id AS ig1_index_grp_id ,ig0.index_id AS index_id , CONV(hex(ig0.exist),16,2) AS exist0 ,CONV(hex(ig1.exist),16,2) AS exist1 ,CONV(hex(ig1.vals),16,2) AS vals ,CONV(hex(ig1.dano),16,2) AS dano FROM index_grp AS ig0 LEFT JOIN " +table_grp +" AS ig1 ON ig1.dataset_id=" +bmf::dataset.at("id") +" AND ig1.`key`=" +to_string(key) +" AND ig0.index_id=ig1.index_id AND ig0.itog_id=ig1.itog_id " +on +" WHERE ig0.`key`=-1 AND ig0.itog_id=" +itog_id +" AND ig0.dataset_id=-1 " +where +";"; sql.empty()){ err("Запрос на выборку"); // // OR (ig1.index_grp_id IS NULL AND ig1.dataset_id IS NULL) OR (ig0.index_grp_id IS NULL AND ig0.dataset_id IS NULL AND ig1.itog_id=" +itog_id +"))
							...
							}else{ //mpre("Список групп " +to_string(INDEX_GRP.size()) +" " +sql ,__LINE__);
							...
							}else if(string sql = "SELECT id ,index_id ,index_grp_id ,grow ,CONV(hex(vals), 16, 2) AS vals FROM " +table_grp +" WHERE dataset_id=" +dataset_id +" AND itog_id=" +itog_id +" AND `key`=" +to_string(key) +" AND index_grp_id IN(" +in +");"; sql.empty()){ err("Запрос на выборку");
							...
							if(int verbose = atoi(bmf::ARGV.end() == bmf::ARGV.find("verbose") ?"" :bmf::ARGV.at("verbose").c_str()); 3 != verbose){ //mpre("Не отображаем подробную информацию" ,__LINE__);
							}else{ mpre(" " +to_string((std::chrono::system_clock::now().time_since_epoch()).count()/1e9 - _microtime) +" Список родительских групп itog_id=" +itog_id +" " +to_string(GRP.size()) +" " ,__LINE__);
							}return false; }()){ err("Уведомление");
						...
						}else{ //mpre("Родитель группы addr_grp=" +addr_grp +" index_grp_id=" +index_grp_id ,__LINE__);
						...
						if(bool ext = [&](bool ext = false){ // Наличие морфа
							...
							if(bool dv = dano.test(nn); false){ err("Значение исходника");
								...
								}else if(v1 = bmf::Val(addr1 ,itog_id ,key ,table_grp ,dv ,ig0_id ,exist ,vals ,BMF_INDEX ,&SIGN ,__LINE__) ;false){ err("Значение страшего морфа");
								...
								}else if(v0 = bmf::Val(addr0 ,itog_id ,key ,table_grp ,dv ,ig0_id ,exist ,vals ,BMF_INDEX ,&SIGN ,__LINE__) ;false){ err("Значение младшего морфа");
						...
						if(auto [ADDR ,next] = [&](int nn =1 ,TMi ADDR = {}){ do{ // Значение роста
							if(bool dv = dano.test(nn); false){ err("Значение исходника");
								...
								if(string addr1 = "11" +adr.substr(1, -1) +addr_grp.substr(1, -1); addr1.empty()){ err("Полный адрес старшего потомка");
								}else if(v1 = bmf::Val(addr1 ,itog_id ,key ,table_grp ,dv ,"" ,exist ,vals ,BMF_INDEX ,&SIGN ,__LINE__) ;false){ err("Значение страшего морфа");
								...
								}else if(v0 = bmf::Val(addr0 ,itog_id ,key ,table_grp ,dv ,"" ,exist ,vals ,BMF_INDEX ,&SIGN ,__LINE__) ;false){ err("Значение страшего морфа");
						...
						if(string str_exist = [&](std::string map = ""){ // Карта наличия
							...
							if(auto _microtime = (std::chrono::system_clock::now().time_since_epoch()).count()/1e9; false){ mpre("ОШИБКА расчета времени эпохи", __LINE__);
							}else if(SQL += ",\n(" +itog_id +" ," +dataset_id +" ," +index_id +" ," +to_string(key) +" ," +grow +" ,'" +list +"' ," +(index_grp_id.empty() ?"NULL" :index_grp_id) +" ,0b" +str_exist +" ,0b" +str_dano +" ,0b" +str_vals +")"; SQL.empty()){ err("Формирование запроса на изменения");
							}else if(!INDEX_GRP.empty()){ //mpre("Только в конце цикла" ,__LINE__);
							}else if(string sql = "INSERT INTO " +table_grp + " (`itog_id` ,`dataset_id` ,`index_id` ,`key` ,`grow` ,`list` ,`index_grp_id` ,`exist` ,`dano` ,`vals`) VALUES " +SQL.substr(1 ,-1) +" ON DUPLICATE KEY UPDATE grow=VALUES(grow) ,list=VALUES(list) ,index_grp_id=VALUES(index_grp_id) ,exist=VALUES(exist) ,dano=VALUES(dano) ,vals=VALUES(vals);"; sql.empty()){ err("Проверка запроса");
							}else if(mysql_query(bmf::mysql ,sql.c_str())){ mpre("ОШИБКА запроса " +sql +"\n" +string(mysql_error(bmf::mysql)) ,__LINE__);
							}else if(SQL = "" ;false){ err("Обнуление списка изменений");
							}else if(int verbose = atoi(bmf::ARGV.end() == bmf::ARGV.find("verbose") ?"" :bmf::ARGV.at("verbose").c_str()); 3 != verbose){ //mpre("Не отображаем подробную информацию" ,__LINE__);
							}else{ mpre(" " +to_string((std::chrono::system_clock::now().time_since_epoch()).count()/1e9 - _microtime) +" Сохранение значений " ,__LINE__);
					...
					}else if(string sql = "SELECT id ,index_id ,CONV(hex(vals), 16, 2) AS vals FROM " +table_grp +" WHERE dataset_id=" +dataset_id +" AND index_id=" +index_id +" AND itog_id=" +itog_id +" AND `key`=" +to_string(key) +";"; sql.empty()){ err("Запрос на выборку");
					...
					}else if(bmf::Progress("Набор:" +dataset_id + itog_str+" Эпоха:" +epoch +" Примеров:" +to_string(key+1) +" Изменений:"+ to_string(err) /*+" (" +to_string(_microtime_) +" сек.)"*/ ,(float)(key+1)/dataset_count, __LINE__); false){ mpre("Индикатор прогресса", __LINE__);
					}else if(std::string dataset_map_id = "index," +index.at("id") +"," +dataset_id; dataset_map_id.empty()){ mpre("ОШИБКА составления идентификатора карты", __LINE__);
					}else if(TMs index_map = bmf::Up(bmf::DATASET_MAP, {{"id", dataset_map_id}}, {}, {}, __LINE__); false){ mpre("ОШИБКА обнолвения карты исходника", __LINE__);
					}else if(std::string map = (index_map.end() == index_map.find("map") ? std::string(dataset_count, '0') : index_map.at("map")); map.empty()){ mpre("ОШИБКА расчета карты", __LINE__);
					}else if(boost::dynamic_bitset<> imap(map); imap.empty()){ mpre("ОШИБКА установки карты морфа", __LINE__);
					...
					if(bmf::ARGV.end() == bmf::ARGV.find("learn")){ //mpre("Режим обучения отключен" ,__LINE__);
						...
						}else if(TMs index_grp = bmf::Up_mysql(table_grp ,{{"dataset_id" ,bmf::dataset.at("id")} ,{"itog_id", itog_id} ,{"index_id" ,index_id} ,{"key" ,to_string(key)}} ,{} ,{} ,__LINE__); index_grp.empty()){ err("Значение группы");
						...
						}else if(string sql = "SELECT * ,CONV(hex(exist), 16, 2) AS exist ,CONV(hex(vals), 16, 2) AS vals ,CONV(hex(dano), 16, 2) AS dano FROM " +table_grp +" WHERE itog_id=" +itog_id +" AND dataset_id=" +bmf::dataset.at("id") +" AND index_id=" +index_id +" AND `key`=" +to_string(key) +";"; sql.empty()){ err("Запрос выборки обновлений");
						...
						if(list = (index_grp.end() == index_grp.find("list") ? "" : index_grp.at("list")); addr_grp.empty()){ //mpre("Группа первого уровня" ,__LINE__);
							...
							}else if(string sql = "SELECT id ,dataset_id ,`key` ,index_id ,list, grow FROM " +table_grp +" WHERE itog_id=" +itog_id +" AND dataset_id=" +bmf::dataset.at("id") +" AND `key`=" +to_string(key) +" AND index_id IN(" +in +");" ;sql.empty()){ err("Запрос на выборку списка значений");
						...
						if(string addr_new = adr +addr_grp.substr(1, -1); addr_new.empty()){ err("Расчет нового адреса");
							...
							}else if(string sql = "INSERT INTO `index` SET grp=" +index_id +" ,dataset_id=" +dataset_id +" ,`key`=" +to_string(key) +" ,addr='" +addr_new +"' ,md5='" +index_md5 +"' ,dano_id=" +dano.at("id") +" ,itog_id=" +itog_id +" ON DUPLICATE KEY UPDATE addr=VALUES(addr);"; sql.empty()){ err("Проверка запроса");
							}else if(mysql_query(bmf::mysql ,sql.c_str())){ mpre("ОШИБКА запроса " +sql +"\n" +string(mysql_error(bmf::mysql)) ,__LINE__);
		...
		if(bmf::ARGV.end() == bmf::ARGV.find("dano")){ //mpre("Данные для расчета не указаны", __LINE__);
		...
		}else if(TMMs ITOG = bmf::List("itog", {}, __LINE__); ITOG.empty()){ err("Список итогов");
			...
			if(nlohmann::json el = js.value(); el.empty()){ mpre("ОШИБКА элемент не найден", __LINE__);
			...
			}else if(TMMs BMF_INDEX = bmf::List_mysql("index", {}, "SELECT *, md5 AS id FROM `index`", __LINE__); BMF_INDEX.empty()){ err("Список морфов");
			}else if(TMs IVAL = [&](TMs IVAL = {}){ for(auto [itog_id ,itog]:ITOG){ // Список итогов
				if(TMMi INDEX_GRP = [&](TMMi INDEX_GRP = {}){ // Список групп
					if(string sql = "SELECT id ,index_id ,CONV(hex(exist),16,2) AS exist FROM index_grp WHERE itog_id=" +itog_id +" AND `key`=-1 AND dataset_id=-1;"; sql.empty()){ err("Запрос на выборку"); // // OR (ig1.index_grp_id IS NULL AND ig1.dataset_id IS NULL) OR (ig0.index_grp_id IS NULL AND ig0.dataset_id IS NULL AND ig1.itog_id=" +itog_id +"))
					}else if(TMMs _INDEX_GRP = bmf::List_mysql("index_grp", {}, sql, __LINE__); _INDEX_GRP.empty()){ mpre("Пустой список выборки " +sql ,__LINE__);
					}else if([&](){ for(auto [id ,index_grp]:_INDEX_GRP){ INDEX_GRP[atoi(id.c_str())] =index_grp; }return INDEX_GRP.empty(); }()){ err("Список групп");
					}else{ //mpre(INDEX_GRP ,"Список групп" ,__LINE__);
					}return INDEX_GRP; }(); INDEX_GRP.empty()){ err("Список групп");
				}else if(TMs GRP =[&](TMs GRP ={}){ for(auto index_grp_ptr =INDEX_GRP.rbegin(); index_grp_ptr !=INDEX_GRP.rend(); index_grp_ptr++){ //for(auto [id ,index_grp]:INDEX_GRP){ // Расчет списка групп
					if(TMs index_grp = index_grp_ptr->second; index_grp.empty()){ err("Значени группы");
					}else if(TMs index = [&](TMs index = {}){ // Морф группы
						if(string index_id = (index_grp.end() == index_grp.find("index_id") ? "" : index_grp.at("index_id")); index_id.empty()){ mpre(index_grp ,"ОШИБКА Идентификатор группового морфа" ,__LINE__);
						}else if(index = bmf::Up("index" ,{{"id" ,index_id}}, {}, {}, __LINE__); index.empty()){ mpre(index_grp ,"Группа" ,__LINE__); mpre("ОШИБКА Групповой морф index_id=" +index_id ,__LINE__);
						}else{ //mpre("Морф группы " +index.at("addr") ,__LINE__);
						}return index; }() ;index.empty()){ err("Морф группы");
					}else if(string addr_grp = (index.end() == index.find("addr") ?"" : index.at("addr")); addr_grp.empty()){ err("Адрес группы");
					}else if(boost::dynamic_bitset<> exist = [&](boost::dynamic_bitset<> exist = {}){ //Карта наличия
						if(string str = (index_grp.end() == index_grp.find("exist") ? "" : index_grp.at("exist")); false){ mpre("ОШИБКА Значение общей карты наличия" ,__LINE__);
						}else if(string map = (string(64 -str.length() ,'0') +str); 64 != map.length()){ err("Полная строка карты");
						}else if(exist = boost::dynamic_bitset(map); 64 != exist.size()){ err("Битовая карта общего налия");
						}else{ //mpre("Карта ключ exist=" +map ,__LINE__);
						}return exist; }(); exist.empty()){ err("Общая карта наличия");
					}else if(string ig0_id =(index_grp.end() ==index_grp.find("id") ?"" :index_grp.at("id")); ig0_id.empty()){ //mpre("Не указана родительская группа" ,__LINE__);
					}else if(string table_grp = "index_grp"; table_grp.empty()){ err("Имя таблицы");
					}else if(string val =[&](boost::dynamic_bitset<> vals ,string val =""){ for(int nn = 63; nn > 0; nn--){ // Расчет группы
						if(!exist.test(nn)){ //mpre(index_grp ,"Морф не создан nn=" +to_string(nn) ,__LINE__);
						}else if(string adr = bmf::Dec2bin(nn); adr.empty()){ err("Двоичный локальный адрес");
						}else if(string dano_id = [&](string dano_id = ""){ // Идентификатор исходника
							if(64 <= nn){ mpre("Адрес группы не вычисляем dv nn=" +to_string(nn) +" adr=" +adr ,__LINE__);
							}else if(string _addr = adr +addr_grp.substr(1 ,-1); _addr.empty()){ err("Адрес морфа");
							}else if(string index_md5 = md5(itog_id +":" +_addr); index_md5.empty()){ err("Расчет идентификатора морфа");
							}else if(TMs _index = [&](TMs _index ={}){ // Морф
								if(_index = BMF_INDEX.end() == BMF_INDEX.find(index_md5) ?_index :BMF_INDEX.at(index_md5); !_index.empty()){ //mpre("Выборка морфа itog_id=" +itog_id +" _addr=" +_addr ,__LINE__);
								}else if(_index = bmf::Up_mysql("index" ,{{"md5" ,index_md5}}, {}, {}, __LINE__); _index.empty()){ mpre("ОШИБКА Выборка из базы" ,__LINE__);
								}else{ //mpre("Выборка из базы" ,__LINE__);
								}return _index; }() ;_index.empty()){ mpre("ОШИБКА Выборка морфа itog_id=" +itog_id +" _addr=" +_addr ,__LINE__);
							}else if(dano_id = _index.end() ==_index.find("dano_id") ?"" :_index.at("dano_id"); dano_id.empty()){ err("Идентификатор исходника");
							}else{ //mpre("Идентификатор исходника" ,__LINE__);
							}return dano_id; }() ;false){ err("Идентификатор исходника");
						}else if(string dv = [&](string dv = "0"){ // Значение исходника
							if(dano_id.empty()){ //mpre("Идентификатор исходника не указан" ,__LINE__);
							}else if(TMs dano = DANO.end() ==DANO.find(dano_id) ?dano :DANO.at(dano_id); dano.empty()){ err("Исходник");
							}else if(string dano_values_id =dano.end() ==dano.find("dano_values_id") ?"" :dano.at("dano_values_id"); dano_values_id.empty()){ err("Идентификатор значения");
							}else if(TMs dano_values =DANO_VALUES.end() ==DANO_VALUES.find(dano_values_id) ?dano_values :DANO_VALUES.at(dano_values_id); dano_values.empty()){ err("Значение");
							}else if(auto vals_ptr = VALS.find(dano_values_id); VALS.end() ==vals_ptr){ //mpre("Список значения не задано");
							}else if(dv = vals_ptr->second.end() ==vals_ptr->second.find(dano_id) ?"0" :vals_ptr->second.at(dano_id); dv.empty()){ mpre("ОШИБКА Значение морфа" ,__LINE__);
							}else{ //mpre(index ,"Корневой морф" ,__LINE__); //mpre(VALS ,"Значения исходников adr=" +adr +" _addr=" +_addr ,__LINE__);
							}return dv; }() ;false){ err("Значение исходника");
						}else if([&](){ // Значение морфа
							//if(mpre("Проверка v1", __LINE__); false){ mpre("ОШИБКА уведомления", __LINE__);
							if(bool v1 = [&](bool v1){ // Значение старшего морфа
								if(string adr1 = "11" +adr.substr(1, -1); adr1.empty()){ err("Локальный адрес");
								}else if(string addr1 = adr1 +addr_grp.substr(1, -1); addr1.empty()){ err("Адрес группы");
								}else if(string index_md5 = md5(itog_id +":" +addr1) ;index_md5.empty()){ err("Идентификатор дочерней группы");
								}else if(BMF_INDEX.end() ==BMF_INDEX.find(index_md5)){ //mpre("Морф не задан" ,__LINE__);
								//}else if(string val1 =GRP.end() ==GRP.find(addr1) ?"" :GRP.at(addr1) ;val1.empty()){ mpre("ОШИБКА Значение группы addr1=" +addr1 ,__LINE__);
								}else if(int _nn = bmf::Bin2dec(adr1); !_nn){ err("Локальный адрес");
								}else if([&](){ // Локальное значение
									if(64 <= _nn){ //mpre("Значение дочерней группы" ,__LINE__);
									//}else if(mpre("Проверка " +to_string(_nn) +" ", __LINE__); false){ mpre("ОШИБКА уведомления", __LINE__);
									}else if(v1 = vals.test(_nn) ;false){ err("Значение дочерней группы");
									}else{ //mpre("Значение локальное" ,__LINE__);
									}return false; }()){ err("Расчет локального значения");
								}else if([&](){ // Значение дочерней группы
									if(64 > _nn){ //mpre("Значение дочерней группы" ,__LINE__);
									}else if(string val =GRP.end() ==GRP.find(addr1) ?"" :GRP.at(addr1) ;val.empty()){ mpre("ОШИБКА Значение дочерней группы addr1=" +addr1 ,__LINE__);
									}else if(v1 = "0" !=val; false){ err("Значение дочерней группы");
									}else{ //mpre("Значение из дочерней группы" ,__LINE__);
									}return false; }()){ err("Расчет локального значения");
								}else{ //mpre("Значение из группы addr1=" +addr1 +" v1=" +string(v1 ?"1" :"0") ,__LINE__);
								}return v1; }("0" != dv); false){ err("Значение старшего морфа");
							//}else if(mpre("Проверка v0", __LINE__); false){ mpre("ОШИБКА уведомления", __LINE__);
							}else if(bool v0 = [&](bool v0){ // Значение старшего морфа
								if(string adr0 = "10" +adr.substr(1, -1); adr0.empty()){ err("Локальный адрес");
								}else if(string addr0 = adr0 +addr_grp.substr(1, -1); addr0.empty()){ err("Адрес группы");
								}else if(string index_md5 = md5(itog_id +":" +addr0) ;index_md5.empty()){ err("Идентификатор дочерней группы");
								}else if(BMF_INDEX.end() ==BMF_INDEX.find(index_md5)){ //mpre("Морф не задан" ,__LINE__);
								}else if(int _nn = bmf::Bin2dec(adr0); !_nn){ err("Локальный адрес");
								}else if([&](){ // Локальное значение
									if(64 <= _nn){ //mpre("Значение дочерней группы" ,__LINE__);
									//}else if(mpre("Проверка _nn" +to_string(_nn) +" ", __LINE__); false){ mpre("ОШИБКА уведомления", __LINE__);
									}else if(v0 = vals.test(_nn) ;false){ err("Значение дочерней группы");
									}else{ //mpre("Значение локальное" ,__LINE__);
									}return false; }()){ err("Расчет локального значения");
								}else if([&](){ // Значение дочерней группы
									if(64 > _nn){ //mpre("Значение дочерней группы" ,__LINE__);
									}else if(string val =GRP.end() ==GRP.find(addr0) ?"" :GRP.at(addr0) ;val.empty()){ err("Значение дочерней группы");
									}else if(v0 = "0" !=val; false){ err("Значение дочерней группы");
									}else{ //mpre("Значение из дочерней группы" ,__LINE__);
									}return false; }()){ err("Расчет локального значения");
								}else{ //mpre("Значение из группы addr0=" +addr0 +" v0=" +string(v0 ?"1" :"0") ,__LINE__);
								}return v0; }("0" != dv); false){ err("Значение младшего морфа");
							}else if(string link = [&](string link = ""){ // Ссылка
								if(1 < adr.length()){ link = adr.substr(1, 1); //mpre("Ссылка из адреса морфа" ,__LINE__);
								}else if(1 < addr_grp.length()){ link = addr_grp.substr(1, 1); // mpre("Ссылка из родительской группы" ,__LINE__);
								}else if(link = (exist.test(0) ?"1" :"0"); link.empty()){ //mpre("Установка ссылки по наличию adr=" +adr +" _link=" +string(_link ?"1" :"0") +" link=" +link ,__LINE__);
								}else{ //mpre("Расчет ссылки");
								}return link; }() ;1 != link.length()){ err("Ссылка")
							//}else if(mpre("Проверка val", __LINE__); false){ mpre("ОШИБКА уведомления", __LINE__);
							}else if(bool v = !v1 & v0 ^!("0" != link); false){ err("Значение");
							}else if(v ?vals.set(nn) :vals.reset(nn) ;false){ err("Установка значения группы");
							}else if(val = (v ?"1" :"0"); val.empty()){ err("Расчет значения группы");
							}else{ //mpre("Расчет морфа itog_id=" +itog_id +" adr=" +adr +" addr_grp=" +addr_grp +" dano_id=" +dano_id +" dv=" +dv +" v1=" +(v1 ?"1" :"0") +" v0=" +(v0 ?"1" :"0") +" link=" +link +" val=" +val +" vals.size()=" +to_string(vals.size()) ,__LINE__);
							}return false; }()){ err("Значение морфа");
						}else{ //mpre("Расчет Группы adr=" +adr +" " ,__LINE__);
						}}return val; }(boost::dynamic_bitset<>(string(64 ,'0'))) ;1 !=val.length()){ err("Значение группы");
					}else if(GRP[addr_grp] = val; GRP.empty()){ err("Сохранение значения группы");
					}else{ //mpre(GRP ,"Расчет группы addr_grp=" +addr_grp ,__LINE__);
					}}return GRP; }(); GRP.empty()){ err("Расчет списка групп");
				//}else if(mpre(GRP ,"Значения групп" ,__LINE__) ;false){ err("Уведомления");
				}else if([&](){ // Значение дерева
					if(GRP.end() == GRP.find("1")){ err("Значение корневой группы");
					}else if(IVAL[itog_id] = GRP.at("1"); IVAL.empty()){ err("Значение дерева");
					}else{ //mpre("Значение дерева");
					}return false; }()){ err("Значение дерева");
				}}return IVAL; }(); IVAL.empty()){ err("Список значения");
			...
			}else if(TMMs ITOG_TITLES = [&](TMMs ITOG_TITLES = {}){ // Заголовки итога
				if(string sql = "SELECT *, value AS id FROM itog_titles"; sql.empty()){ err("Составление запроса на выборку групп итогов");
				}else if(ITOG_TITLES = bmf::List_mysql("itog_titles", {} ,sql, __LINE__); false){ err("Список исходников");
				}else{ //mpre(ITOG_TITLES ,"Заголовки итогов" ,__LINE__);
				}return ITOG_TITLES; }() ;false){ err("Заголовки итога");
				...
				}else if(string name =(itog_values.end() ==itog_values.find("name") ?val_itr.first :itog_values.at("name")) ;name.empty()){ err("Имя значения");
				}else if(string title =(itog_titles.end() == itog_titles.find("name") ? value : itog_titles.at("name")); title.empty()){ err("Значение");
				}else if(j[key]["itog"][name] = title; j.empty()){ err("Формирование json");
				}else{ //mpre(itog_values ,"Установка значения" ,__LINE__);
